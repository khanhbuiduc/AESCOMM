...
namespace AES2
{
    public class AES128Core
    {
        ...
        // Key expansion for AES-128 (modified for 128-bit key)
        public static uint[] KeyExpansion(uint[] Key)
        {
            uint[] w = new uint[44]; // 11 rounds * 4 words per round = 44 words

            // Copy initial key (4 words for 128-bit key)
            for (int i = 0; i < 4; i++)
            {
                w[i] = Key[i];
            }
