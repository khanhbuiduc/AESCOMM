...
namespace AES2
{
    public class AES192Core
    {
        // Constants (S-box, InvS-box, Rcon) - same as AES256
        ...
        // Core helper functions - same as AES256
        ...
        // Key expansion for AES-192 (modified for 192-bit key)
        public static uint[] KeyExpansion(uint[] Key)
        {
            uint[] w = new uint[52]; // 13 rounds * 4 words per round = 52 words

            // Copy initial key (6 words for 192-bit key)
            for (int i = 0; i < 6; i++)
            {
                w[i] = Key[i];
            }
        ...
        // Standard AES operations - same as AES256
        ...
        // Galois Field operations for MixColumns - same as AES256
        ...
        // MixColumns operation - same as AES256
        ...
        // AES-192 encryption (12 rounds instead of 14)
        public static uint[] Encrypt(uint[] state, uint[] key)
        {
        ...
        // Inverse operations for decryption - same as AES256
        ...
        // AES-192 decryption (12 rounds instead of 14)
        public static uint[] Decrypt(uint[] C, uint[] key)
        {
            uint[] w = KeyExpansion(key);
            uint[] state = AddRoundKey(C, w, 12);

            // 11 main rounds (inverse order)
            for (int j = 11; j >= 1; j--)
            {
