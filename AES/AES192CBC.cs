...
namespace AES2
{
    public class Aes192Helper
    {
        ...
        // Encrypt a message using AES-192 in CBC mode
        public static uint[][] EncryptCBC(string message, uint[] key, uint[] iv)
        {
            ...
            for (int i = 0; i < blocks.Length; i++)
            {
                ...
                uint[] encryptedBlock = AES192Core.Encrypt(xoredBlock, key);
        ...
        // Decrypt a message encrypted with AES-192 in CBC mode
        public static string DecryptCBC(uint[][] encryptedBlocks, uint[] key)
        {
            ...
            {
                uint[] decryptedBlock = AES192Core.Decrypt(encryptedBlocks[i], key);
        ...
        {
            return AES192Core.Encrypt(state, key);
        }

        public static uint[] GiaimaAES(uint[] C, uint[] key)
        {
            return AES192Core.Decrypt(C, key);
        }
    }
}
